...
parser.add_argument('--gt_type', type=str, default='direction',
                    help='Ground truth type: direction')
...
loss_data = pd.DataFrame(columns=['Epoch', 'Total Loss', 'CE Loss', 'LR'])
...
for epoch in range(301):
    ...
    for input_data, truth, filename in tqdm(train_data_loader, desc=f'Epoch {epoch_true + 1}/301', unit='batch',
        ...
        optimizer.zero_grad()
        output, _ = model(input_data)  # 输出 shape: (batch, 5, height, width)

        loss = criterion(output, truth.long())
        loss.backward()
        optimizer.step()
    ...
    avg_CE_loss = CE_loss / len(train_data_loader)

    print(f'Epoch {epoch_true + 1}, LR: {learning_rate}, Loss: {avg_total_loss}, CELoss: {avg_CE_loss}')
