...
class UNet_ConvLSTM(nn.Module):
    def __init__(self, n_channels, use_LSTM=True, parallel_encoder=False, factor=0, lstm_layers=2):
    ...
    def forward(self, x, prev_hidden=None):
        ...
        if self.use_LSTM:
            if self.parallel_encoder:
                ...
                factor = t if self.factor == 0 else self.factor
        ...
        x = self.up4(x, x1[:b, :, :, :])
        x = self.outc(x)
        # 如果需要获得归一化的类别概率，可以使用 softmax，如下：
        # x = F.softmax(x, dim=1)
        return x, test
