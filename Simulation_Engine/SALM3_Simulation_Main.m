% Generating training dataset for SALM 3
clc; clear; close all;
rng(666);
...
for imgIdx = 1:NUM_SAMPLES
    ...
    % Per-segment HR frames and union structure
    ...
    for tIdx = 1:num_tracks
        ...
        % Per-segment bright pulses
        pulseMidIdx = zeros(1, NUM_SEGMENTS);
        for s = 1:NUM_SEGMENTS
            segBrightHR = zeros(HR_SIZE);
            pts = groups{s}; brs = brightness_groups{s};

            % write base brightness
            for k = 1:size(pts,1)
                segBrightHR(pts(k,1), pts(k,2)) = brs(k,1);
            end

            % choose middle pulse index in [20%, 80%]
            if isempty(pts)
                pulseMidIdx(s) = 1;
            else
                lo = max(1, round(size(pts,1)*0.2));
                hi = max(1, round(size(pts,1)*0.8));
                if hi < lo, hi = lo; end
                pulseMidIdx(s) = randi([lo, hi]);
            end

            % three pulses: start / middle / end, with boost factor
            if ~isempty(pts)
                maxVal = max(brs);
                idx0 = 1;
                idxM = max(1, min(pulseMidIdx(s), size(pts,1)));
                idxE = size(pts,1);

                % middle pulse
                if idxM ~= 1
                    segBrightHR(pts(idxM,1), pts(idxM,2)) = maxVal * (3.5 + 1*rand());
                else
                    segBrightHR(pts(idxM,1), pts(idxM,2)) = maxVal;
                end
                % start and end pulses
                segBrightHR(pts(idx0,1), pts(idx0,2)) = maxVal * (3.5 + 1*rand());
                segBrightHR(pts(idxE,1), pts(idxE,2)) = maxVal * (3.5 + 1*rand());
            end
        ...
        %% Per-segment speeds
        PIXEL_DISTANCEWeight = 2.3; 
        for s = 1:NUM_SEGMENTS
            pts = groups{s};
            if isempty(pts), continue; end

            pm = max(1, min(pulseMidIdx(s), size(pts,1)));
            ...
            for kk = 1:2
                ...
                if isempty(subPts), continue; end
                ...
                speed = total_distance / denom;

                for l = 1:size(subPts,1)
                    speed_map(subPts(l,1), subPts(l,2)) = speed;
                    GT(subPts(l,1), subPts(l,2))        = 255;
                end
            end
        ...
        for l = 1:size(points,1)-1
            ...
            ang = atan2(-dxx, dyy);
            if ang == 0
                ang = (dxx == 0 && dyy == 0) * 0 + (dxx ~= 0 || dyy ~= 0) * 10;
            end
    ...
    %% Downsample frames to LR and add Gaussian noise
    NoiseVar = 8e-3 * rand();
    for s = 1:NUM_SEGMENTS
