...
CHECKPOINT = f"Checkpoint/{TestMode}_Checkpoint_VelocityMap.pkl"
INPUT_H5   = f"TestData/{TestMode}_TestData.H5"
OUTPUT_PNG = f"{TestMode}_Results/VelocityMap.png"
...
def main():
    ...
    with torch.no_grad():
        for sample in tqdm(data_loader, desc="Inference", unit="batch"):
            ...
            out = model(sample_up)[0].squeeze()   # (IM_SIZEX, IM_SIZEY)

            if EPS > 0:
                mask = (out.abs() > EPS) & torch.isfinite(out)
            else:
                mask = (out != 0) & torch.isfinite(out)

            group_sum[mask]   += out[mask]
    ...
    # per-pixel average over non-zero counts
